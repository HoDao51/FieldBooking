<div id="createModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
    <div class="relative w-full max-w-xl rounded-xl bg-white p-6 shadow-xl">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-800">Thêm loại sân</h2>
            <button type="button" onclick="closeModal('createModal')" class="text-xl text-red-500 hover:text-red-700">
                ×
            </button>
        </div>

        <form action="{{ route('loaiSan.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="mb-1 block text-gray-600">Tên loại sân</label>
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Ví dụ: Sân 5 người"
