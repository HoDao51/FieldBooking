    ...
    <div class="pl-2 pb-6">
        ...
        <div class="space-y-8">
            @forelse($mostBookedFields as $facilityName => $fields)
                <div id="facility-{{ collect($fields)->first()->Fields->facility_id }}"
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            @foreach ($fields as $index => $stat)
                                <div
                                    class="flex gap-4 h-full block relative bg-white border border-gray-200 rounded-xl p-4">
                                    <div class="w-24 h-24 flex-shrink-0">
                                        @if ($stat->Fields->images->count() > 0)
                                            <img src="{{ asset('storage/' . $stat->Fields->images->first()->name) }}"
                                                alt="Sân"
                                                class="w-full h-full object-cover rounded-lg border border-gray-200 group-hover:opacity-90 transition">
                                        @else
                                            <div
                                                class="w-full h-full rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 border border-gray-200">
                                    ...
                                    <div class="flex-1 flex flex-col justify-center min-w-0">
                                        <h3 class="font-bold text-gray-800 text-lg mb-1 leading-tight truncate"
                                            title="{{ $stat->Fields->name }}">{{ $stat->Fields->name }}</h3>
                                        ...
                                        <div class="mt-auto flex">
                                            <span
                                                class="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 text-sm font-bold px-3 py-1 rounded-lg border border-blue-100">
