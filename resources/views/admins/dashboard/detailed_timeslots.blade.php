    ...
    <div class="pl-2 pb-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="flex items-center gap-3 text-2xl font-bold text-gray-800">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 text-purple-500" viewBox="0 0 24 24" fill="none"
        ...
        <div class="space-y-8">
            @forelse($topTimeSlotsByFacility as $facilityName => $fieldsGroup)
                <div id="facility-{{ collect($fieldsGroup)->first()->first()->facility_id }}"
                                ...
                                <div class="border border-gray-200 rounded-xl p-5 bg-gray-50/50">
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-4">
                                        @foreach ($slots as $index => $slot)
                                            <div
                                                class="bg-white border border-gray-200 rounded-xl p-3 flex flex-col items-center shadow-sm relative overflow-hidden">
                                                ...
                                                <span class="font-bold text-purple-700 text-base mt-1 tracking-wider">
                                                    {{ \Carbon\Carbon::parse($slot->TimeSlot->startTime)->format('H:i') }} -
                                                    {{ \Carbon\Carbon::parse($slot->TimeSlot->endTime)->format('H:i') }}
                                                </span>
                                                <div class="mt-1 flex">
                                                    <span
                                                        class="inline-flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1 rounded-lg border border-blue-100">
